...
export class AppointmentsService {
  ...
  async create(dto: { patientId: string; doctorId: string; date: string; startTime: string; duration: number; billingMode?: string; procedure?: string; procedureAr?: string; notes?: string; sterilizationBuffer?: number; toothNumber?: number }) {
  ...
  async update(id: string, dto: Record<string, unknown>, doctorIdFilter?: string) {
    const appointment = await this.appointmentModel.findById(id);
    if (!appointment) throw new NotFoundException('Appointment not found');
    if (doctorIdFilter && !appointment.doctorId.equals(new Types.ObjectId(doctorIdFilter))) {
      throw new ForbiddenException('Not your appointment');
    }
    const date = (dto.date as string) ?? appointment.date;
    const startTime = (dto.startTime as string) ?? appointment.startTime;
    const duration = (dto.duration as number) ?? appointment.duration;
    const endTime = addMinutesToTime(startTime, duration);
    const doctorId = dto.doctorId != null
      ? new Types.ObjectId(dto.doctorId as string)
