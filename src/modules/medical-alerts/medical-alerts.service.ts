...
export class MedicalAlertsService {
  ...
  async findByPatient(patientId: string) {
    return this.medicalAlertModel.find({ patientId: toObjectIdOrThrow(patientId, 'patientId') }).sort({ createdAt: -1 }).lean();
  }
  ...
  async remove(id: string) {
    const deleted = await this.medicalAlertModel.findByIdAndDelete(toObjectIdOrThrow(id, 'id'));
